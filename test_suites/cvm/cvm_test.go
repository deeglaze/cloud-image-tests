...
// Returns the TMPT_PUBLIC result of creating the L-1 endorsement key.
func createTpmL1EK(t testing.TB) []byte {
	// L-1 template from TCG EK credential profile, which is what is used for the initial version
	// of the SVSM attestation protocol.
	l1public := []byte{
	...
	pub, err := tpm2.DecodePublic(l1public)
	if err != nil {
		t.Fatalf("parsing input template: %v", err)
	}
	...
	defer rw.Close()
	hnd, public, _, _, _, _, err := tpm2.CreatePrimaryEx(
		rw, owner, sel, parentPassword, ownerPassword, pub)
	if err != nil {
		t.Fatalf("Could not create primary object: %v", err)
	}
	defer tpm2.FlushContext(rw, hnd)

	return public
}

// Returns the SEV-SNP attestation report and manifest binding the SVSM vTPM EK to the
// CVM.
func svsmVtpmAttestation(t testing.TB, nonce []byte) *report.Response {
	client, err := linuxtsm.MakeClient()
	if err != nil {
		t.Fatalf("Could not create configfs-tsm report client: %v", err)
	}
	oreport, err := report.Create(client, &report.Request{
		InBlob:          nonce,
		ServiceProvider: "svsm",
		ServiceGuid:     "c476f1eb-0123-45a5-9641-b4e7dde5bfe3",
	})
	if err != nil {
		t.Fatalf("Could not create configfs-tsm report: %v", err)
	}
	defer oreport.Destroy()
	response, err := oreport.Get()
	if err != nil {
		t.Fatalf("could not get SVSM attestation report: %v", err)
	}
	return response
}

func TestSNPSVSMVTPML1EK(t *testing.T) {
	nonce := make([]byte, 64)
	response := svsmVtpmAttestation(t, nonce)
	ekpub := createTpmL1EK(t)
	// The manifest and the EK pub are expected to be the same.
	if !bytes.Equal(ekpub, response.ManifestBlob) {
		t.Fatalf("TPM L-1 EK differs from SVSM attested vTPM EK")
	}
	report, err := sevabi.ReportToProto(response.OutBlob)
	if err != nil {
		t.Fatalf("SEV-SNP attestation report parse failure: %v", err)
	}
	if report.Vmpl != 0 {
		t.Errorf("SVSM VMPL was %d, want 0", report.Vmpl)
	}
	h := sha512.New()
	h.Write(nonce)
	h.Write(ekpub)
	expectReportData := h.Sum(nil)
	if !bytes.Equal(report.ReportData, expectReportData) {
		t.Error("Manifest binding did not match attestation report")
	}
}
